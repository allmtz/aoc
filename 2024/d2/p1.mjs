const input = `Game 1: 6 green, 3 blue; 3 red, 1 green; 4 green, 3 red, 5 blue
...
const rex = /(\d+)\s(red|green|blue)/gi;
  ...
  rounds.forEach((r, i) => {
    ...
    if (matches) {
      ...
      for (const m of matches) {
        ...
        count[color] += Number(amount);
        if (color[color] > max[color]) {
          // break out to move on to the next round
          break;
        }
      }
    }
    ...
    let roundIsValid = true;
    for (const color in count) {
      if (count[color] > max[color]) {
        roundIsValid = false;
        break;
      }
    }

    // if a round is invalid -> move on to the next line
    if (roundIsValid === false) {
      lineIsValid = false;
      // console.log(lineIdx);
      return;
    }
